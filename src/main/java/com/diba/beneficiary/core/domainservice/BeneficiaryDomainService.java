...
public class BeneficiaryDomainService {
    ...
    public BeneficiaryDomainService(IEventStoreDBRepository<Beneficiary, UUID> esdbRepo,
                                    BeneficiaryLocalRepository repo) {
        _esdbRepo = esdbRepo;
        _localRepo = repo;
    ...
    public CompletableFuture<ServiceResult<BeneficiaryCreatedDto>> CreateNewBeneficiary(CreateOne data) throws BeneficiaryException {
        ...
        Beneficiary.validateBusinessCode(data.getBusinessCode());
        ...
        CompletableFuture.runAsync(() -> {
            ...
            try {
                dmodel = Beneficiary.create(id, data.getBeneficiaryName(), data.getBeneficiaryNameEn(),
                        data.getBusinessCode(), data.getBeneficiaryRoles(), data.getBeneficiaryType(),
                        data.getNationality(), LocalDateTime.now(), LocalDateTime.now().plusDays(30),
                        LocalDateTime.now(), data.getBourseCode(), data.getTradeCode(), data.getIgmcCode(),
                        data.getBillCode(), data.getAddress(), data.getPostalCode(), data.getPhoneNumber(),
    ...
    public CompletableFuture<ServiceResult<BeneficiaryUpdatedDto>> updateBeneficiary(UpdateOne update) throws BeneficiaryException {
        ...
        Beneficiary.validateBusinessCode(update.getBusinessCode());
        var bene = checkIfExistInLocalDB(update.getIid());
        checkIfNotExistInLocalDB(update.getBusinessCode());
        CompletableFuture.runAsync(() -> {
            ETag result;
            try {
                result = _esdbRepo.getAndUpdate(current -> current.update(update),
                        UUID.fromString(updateDto.getIid()), update.getExpectedVersion());
            } catch (Exception e) {
                throw new RuntimeException(e);
            }
    ...
    public CompletableFuture<ServiceResult<String>> changeStatus(ChangeStatus status) throws BeneficiaryException {
        ...
        CompletableFuture.runAsync(() -> {
            try {
                _esdbRepo.getAndUpdate(current -> current.ChangeStatus(status), UUID.fromString(status.getIid()),
    ...
    public CompletableFuture<ServiceResult<String>> deleteBeneficiary(DeleteBeneficiary delete) throws BeneficiaryException {
        ...
        CompletableFuture.runAsync(() -> {
            try {
                _esdbRepo.getAndUpdate(current -> current.DeleteBeneficairy(delete),
                        UUID.fromString(delete.getBeneficiaryId()), delete.getVersion());
            } catch (Exception e) {
                throw new RuntimeException(e);
            }
    ...
    public CompletableFuture<ServiceResult<String>> addProductToBeneficiary(AddProductToBeneficiary addProduct) throws BeneficiaryException {
        CompletableFuture<ServiceResult<String>> future = new CompletableFuture<>();
        checkIfExistInLocalDB(addProduct.getBeneficiaryId().toString());
        CompletableFuture.runAsync(() -> {
            try {
                _esdbRepo.getAndUpdate(current -> current.addProduct(addProduct),
                        UUID.fromString(addProduct.getBeneficiaryId()), addProduct.getExpectedVersion());
            } catch (Exception e) {
                throw new RuntimeException(e);
            }
            future.complete(ServiceResult.success(addProduct.getBeneficiaryId()));
        });
        return future;
    }
